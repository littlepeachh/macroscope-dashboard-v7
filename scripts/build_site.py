...
# v7.3: repair overview card layout while preserving all prior market and Treasury history fixes.
HTML = r'''<!doctype html>
<html lang="zh-CN">
<head>
...
.tabs{display:flex;gap:8px;overflow:auto;padding:18px 0 12px}.tab{border:0;background:#e8edf6;color:#55627a;padding:10px 15px;border-radius:11px;font-weight:700;cursor:pointer;white-space:nowrap}.tab.active{background:var(--navy);color:#fff}.panel{display:none}.panel.active{display:block}.grid{display:grid;gap:16px}.g2{grid-template-columns:repeat(2,minmax(0,1fr))}.g3{grid-template-columns:repeat(3,minmax(0,1fr))}.g4{grid-template-columns:repeat(4,minmax(0,1fr))}.kpis{grid-template-columns:repeat(6,minmax(0,1fr));margin-bottom:16px}.overview-layout{display:grid;grid-template-columns:minmax(0,1.7fr) minmax(360px,.8fr);gap:16px;align-items:start}.overview-index-card,.overview-breadth-card{min-width:0}.overview-index-card .asset-grid{grid-template-columns:repeat(auto-fit,minmax(250px,1fr))}.card{background:var(--panel);border:1px solid var(--line);border-radius:18px;box-shadow:var(--shadow);padding:17px;min-width:0}.card h3{margin:0 0 12px;font-size:16px}.card h3 small{font-weight:400;color:var(--muted);margin-left:6px}.chart{height:380px}.chart.tall{height:470px}.chart-grid{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:16px}.market-chart-card{background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:14px;box-shadow:var(--shadow);min-width:0}.market-chart-head{display:flex;justify-content:space-between;align-items:flex-start;gap:12px;margin-bottom:4px}.market-chart-title{font-size:15px;font-weight:800}.market-chart-meta{font-size:11px;color:var(--muted);margin-top:3px}.market-chart-latest{text-align:right;font-size:13px;font-weight:800;white-space:nowrap}.market-single-chart{height:315px}.chart-placeholder{height:100%;display:flex;align-items:center;justify-content:center;color:var(--muted);font-size:12px;background:#f8faff;border-radius:10px;border:1px dashed var(--line);padding:18px;text-align:center}.kpi{background:var(--panel);border:1px solid var(--line);border-radius:16px;padding:15px;box-shadow:var(--shadow);min-height:112px}.kpi-label{font-size:12px;color:var(--muted);font-weight:700}.kpi-value{font-size:25px;font-weight:800;margin-top:8px;line-height:1}.kpi-note{font-size:11px;color:#8a94a8;margin-top:10px}.positive,.up{color:var(--up)!important}.negative,.down{color:var(--down)!important}.neutral{color:var(--muted)!important}.kpi-value,.asset-price,.asset-metrics b,.multiple{transition:color .25s ease}.asset-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}.asset{border:1px solid var(--line);border-radius:15px;padding:14px;background:linear-gradient(180deg,#fff,#fbfcff);min-width:0;overflow:hidden}.asset-top{display:flex;justify-content:space-between;align-items:flex-start;gap:8px;min-width:0}.asset-top>div{min-width:0}.asset-top>b{flex:0 0 auto;white-space:nowrap}.asset-name{font-weight:800;font-size:14px;overflow-wrap:anywhere}.asset-symbol{font-size:11px;color:var(--muted);margin-top:3px;overflow-wrap:anywhere}.asset-price{font-size:24px;font-weight:800;margin:12px 0;overflow-wrap:anywhere}.asset-metrics{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:6px}.asset-metrics div{background:#f4f6fa;border-radius:9px;padding:7px;min-width:0;overflow:hidden}.asset-metrics span{display:block;font-size:10px;color:var(--muted);white-space:nowrap}.asset-metrics b{display:block;font-size:12px;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}.hint{background:#f6f8fc;border:1px solid var(--line);border-radius:12px;padding:11px 13px;color:#66738a;font-size:12px;line-height:1.6;margin:12px 0}.valuation-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:12px}.valuation-card{border:1px solid var(--line);border-radius:14px;padding:13px;cursor:pointer}.valuation-card.selected{border-color:var(--blue);box-shadow:0 0 0 2px rgba(49,103,227,.10)}.multiple{font-size:25px;font-weight:800;margin:9px 0}.stat-row{display:grid;grid-template-columns:repeat(3,1fr);gap:5px}.stat{background:#f5f7fb;border-radius:7px;padding:6px;font-size:10px;color:var(--muted)}.stat b{display:block;color:var(--ink);font-size:11px;margin-top:2px}.bar{height:5px;background:#edf0f5;border-radius:5px;margin-top:10px;overflow:hidden}.bar i{display:block;height:100%;background:linear-gradient(90deg,var(--down),var(--amber),var(--up))}.table-wrap{overflow:auto;border:1px solid var(--line);border-radius:13px}table{width:100%;border-collapse:collapse;font-size:12px;min-width:760px}th{background:#f3f6fb;color:#59667d;text-align:left;padding:10px;position:sticky;top:0}td{border-top:1px solid var(--line);padding:9px 10px}tr:hover td{background:#fafcff}.source-grid{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:12px}.source-card{border:1px solid var(--line);border-radius:14px;padding:13px;background:#fff}.source-status{font-weight:800}.source-status.success,.source-status.cached{color:var(--down)}.source-status.failed{color:var(--up)}.source-status.partial{color:var(--amber)}.empty{display:none;padding:28px;text-align:center;color:var(--muted)}.footer{padding:25px 0;color:var(--muted);font-size:11px;line-height:1.7}
@media(max-width:1250px){.overview-layout{grid-template-columns:1fr}.overview-index-card .asset-grid{grid-template-columns:repeat(3,minmax(0,1fr))}}
@media(max-width:1100px){.kpis{grid-template-columns:repeat(3,1fr)}.asset-grid,.valuation-grid{grid-template-columns:repeat(2,1fr)}.overview-index-card .asset-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.g2,.g3,.g4{grid-template-columns:1fr}.source-grid{grid-template-columns:1fr 1fr}.chart-grid{grid-template-columns:1fr}}
@media(max-width:650px){.wrap{padding:12px}.hero{padding:22px 18px;border-radius:18px}.hero h1{font-size:23px}.kpis{grid-template-columns:repeat(2,1fr)}.asset-grid,.valuation-grid,.source-grid,.overview-index-card .asset-grid{grid-template-columns:1fr}.chart{height:330px}.market-single-chart{height:290px}}
</style>
</head>
<body><div class="wrap">
...
<section class="panel active" id="overview"><div class="grid kpis" id="overviewKpis"></div><div class="overview-layout"><div class="card overview-index-card"><h3>主要指数最新实际点位</h3><div id="overviewMarketCards" class="asset-grid"></div></div><div class="card overview-breadth-card"><h3>A股市场温度</h3><div id="overviewBreadth" class="chart"></div></div></div></section>
...
<section class="panel" id="global"><div class="card"><h3>全部指数最新行情 <small>按实际点位显示</small></h3><div id="globalCards" class="asset-grid"></div></div><div style="margin-top:16px"><h3 style="margin:0 0 12px">全部指数走势图 <small style="font-weight:400;color:var(--muted)">每个指数单独成图，Y轴为实际点位</small></h3><div id="indexChartGrid" class="chart-grid"></div></div><div class="card" style="margin-top:16px"><h3>美国国债收益率 <small>完整历史 · 可选择1年/5年/10年/全部</small></h3><div id="treasuryKpis" class="grid g2" style="margin-bottom:12px"></div><div id="treasuryChart" class="chart"></div></div><div class="card" style="margin-top:16px"><h3>全部股票最新行情 <small>包含美光、三星电子、SK海力士</small></h3><div id="techCards" class="asset-grid"></div></div><div style="margin-top:16px"><h3 style="margin:0 0 12px">全部股票走势图 <small style="font-weight:400;color:var(--muted)">每只股票单独成图，Y轴为实际价格</small></h3><div id="stockChartGrid" class="chart-grid"></div></div></section>
...
function renderGlobal(){const cards=DATA.market_cards;const indices=cards.filter(c=>['US_INDEX','FX_INDEX','CN_INDEX'].includes(c.market));const stocks=cards.filter(c=>['US_EQUITY','KR_EQUITY'].includes(c.market));document.getElementById('globalCards').innerHTML=indices.map(assetCard).join('');document.getElementById('techCards').innerHTML=stocks.map(assetCard).join('');renderMarketChartGrid('indexChartGrid',indices,'index-chart',520);renderMarketChartGrid('stockChartGrid',stocks,'stock-chart',520);const g=DATA.global_macro,g2=g.filter(x=>x.series==='DGS2'),g10=g.filter(x=>x.series==='DGS10'),t2=latestDelta(g2,'value_pct'),t10=latestDelta(g10,'value_pct');document.getElementById('treasuryKpis').innerHTML=[kpi('美国2年期国债收益率',fmt(t2.v),'%',noteWithDelta(t2.row,'trade_date',t2.delta),cls(t2.delta)),kpi('美国10年期国债收益率',fmt(t10.v),'%',noteWithDelta(t10.row,'trade_date',t10.delta),cls(t10.delta))].join('');if(g.length)Plotly.newPlot('treasuryChart',[{x:g2.map(x=>cnDate(x.trade_date)),y:g2.map(x=>x.value_pct),name:'美国2年期',type:'scattergl',mode:'lines',connectgaps:false,line:{color:C.blue,width:1.8}},{x:g10.map(x=>cnDate(x.trade_date)),y:g10.map(x=>x.value_pct),name:'美国10年期',type:'scattergl',mode:'lines',connectgaps:false,line:{color:C.purple,width:2.1}}],layout({margin:{l:58,r:28,t:28,b:48},xaxis:{type:'date',showgrid:false,rangeselector:{x:0,y:1.14,xanchor:'left',buttons:[{count:1,label:'1年',step:'year',stepmode:'backward'},{count:5,label:'5年',step:'year',stepmode:'backward'},{count:10,label:'10年',step:'year',stepmode:'backward'},{count:20,label:'20年',step:'year',stepmode:'backward'},{step:'all',label:'全部'}]}},yaxis:{title:'收益率（%）',gridcolor:C.grid,autorange:true,rangemode:'normal'},hovermode:'x unified'}),CONFIG)}
