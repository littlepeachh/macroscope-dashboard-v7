...
# v6.0.3: actual-value y-axes and one independent chart for every index and stock.
HTML = r'''<!doctype html>
<html lang="zh-CN">
<head>
...
<section class="panel" id="global"><div class="card"><h3>全部指数最新行情 <small>按实际点位显示</small></h3><div id="globalCards" class="asset-grid"></div></div><div style="margin-top:16px"><h3 style="margin:0 0 12px">全部指数走势图 <small style="font-weight:400;color:var(--muted)">每个指数单独成图，Y轴为实际点位</small></h3><div id="indexChartGrid" class="chart-grid"></div></div><div class="card" style="margin-top:16px"><h3>美国国债收益率</h3><div id="treasuryKpis" class="grid g2" style="margin-bottom:12px"></div><div id="treasuryChart" class="chart"></div></div><div class="card" style="margin-top:16px"><h3>全部股票最新行情 <small>包含美光、三星电子、SK海力士</small></h3><div id="techCards" class="asset-grid"></div></div><div style="margin-top:16px"><h3 style="margin:0 0 12px">全部股票走势图 <small style="font-weight:400;color:var(--muted)">每只股票单独成图，Y轴为实际价格</small></h3><div id="stockChartGrid" class="chart-grid"></div></div></section>
...
function renderMarketChartGrid(containerId,cards,prefix,maxRows=520){const container=document.getElementById(containerId);if(!container)return;container.innerHTML=cards.map(card=>{const id=`${prefix}-${safeId(card.symbol)}`;const color=cls(card.daily_pct);return `<div class="market-chart-card"><div class="market-chart-head"><div><div class="market-chart-title">${card.name}</div><div class="market-chart-meta">${card.symbol} · ${marketUnit(card)} · ${cnDate(card.trade_date)}</div></div><div class="market-chart-latest ${color}">${fmt(card.close,card.currency==='KRW'?0:2)}<br><span style="font-size:11px">${signed(card.daily_pct)}</span></div></div><div id="${id}" class="market-single-chart"></div></div>`}).join('');cards.forEach(card=>{const rows=marketRows(card.symbol,maxRows);if(!rows.length)return;const id=`${prefix}-${safeId(card.symbol)}`;const direction=Number(card.daily_pct)>0?C.up:Number(card.daily_pct)<0?C.down:C.blue;const unit=marketUnit(card);const values=rows.map(r=>Number(r.close));const trace={x:rows.map(r=>cnDate(r.trade_date)),y:values,name:card.name,mode:'lines',line:{color:direction,width:2.2},hovertemplate:`%{x}<br>%{y:${marketTickFormat(card)}} ${unit}<extra>${card.name}</extra>`};const latestPoint={x:[cnDate(rows[rows.length-1].trade_date)],y:[Number(rows[rows.length-1].close)],name:'最新',mode:'markers',marker:{color:direction,size:7},hovertemplate:`最新 %{x}<br>%{y:${marketTickFormat(card)}} ${unit}<extra></extra>`};Plotly.newPlot(id,[trace,latestPoint],layout({showlegend:false,margin:{l:62,r:18,t:12,b:45},xaxis:{gridcolor:C.grid,showgrid:false},yaxis:{title:unit||'实际值',gridcolor:C.grid,autorange:true,rangemode:'normal',tickformat:marketTickFormat(card),separatethousands:true}}),CONFIG)})}
